...
class BeamNGROS:
    def __init__(self, map_name, start_pos, start_quat, config_path, target_WP=None):
        ...
        self.gridmap_pub = rospy.Publisher(
            "/elevation_mapping/elevation_map_raw", GridMap, queue_size=1
        )
        # set up odom publisher
        self.odom_pub = rospy.Publisher(
            "/mavros/local_position/odom", Odometry, queue_size=1
        )
        ...
        self.vesc_state_pub = rospy.Publisher(
            "/sensors/core", VescStateStamped, queue_size=1
        )
        # set up transform broadcaster:
        self.tf_broadcaster = tf.TransformBroadcaster()
        # set up control subscriber AckermannDriveStamped
        self.control_sub = rospy.Subscriber(
            "pass/control", AckermannDriveStamped, self.control_callback
        )
        # set up mavros control sub:
        self.mavros_control_sub = rospy.Subscriber(
            "/mavros/manual_control/send", ManualControl, self.mavros_control_callback
        )
    ...
    def publish_state(self, state, timestamp):
        orientation_quat = Quaternion(
            *quaternion_from_euler(state[3], state[4], state[5])
        )
...
if __name__ == "__main__":
    ...
    waypoint_path = (
        str(Path(os.getcwd()).parent.absolute()) + "/BeamNGRL/utils/waypoint_files/"
    )
    target_WP = np.load(waypoint_path + "WP_file_offroad.npy")
    bingchilling = BeamNGROS(
        map_name, start_point, start_quat, config_path, target_WP=target_WP
    )
