...
parser.add_argument(
    ...
    help="name of the HAL config file to use",
)
parser.add_argument(
    "--config_name",
    type=str,
    default="gym_experiment.yaml",
    help="name of the config used for experiment specifics",
)
parser.add_argument(
    "--remote",
    type=bool,
...
)  ## anything experiment specific should be put here

env = gym.make(
    "offroad-small-island-v0",
...
with torch.no_grad():
    for model_name in env.Config["model"]:
        ...
        while not done and counter < 1000:
            ...
            actions = (
                model.forward(
                    torch.from_numpy(obs["path"]).to(device=device, dtype=dtype),
                    torch.from_numpy(obs["state"]).to(device=device, dtype=dtype),
                )
                .cpu()
                .numpy()
            )
