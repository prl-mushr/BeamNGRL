...
)
{
   ...
   layerDiffuseColor.rgba *= diffuseMaterialColor0.rgba;
   layerDiffuseColor.a = saturate(layerDiffuseColor.a);
   ...
   lightinfoUncondition(lightInfoBuffer.Sample(defaultSamplerRT, uvScene), d_lightcolor, d_NL_Att, d_specular);
    float4 outputFinalColor = max(0, materialDiffuseColor);
   outputFinalColor = toLinearColor(outputFinalColor) * float4(d_lightcolor, 1.0);
   outputFinalColor.rgb += materialEmmisive;
   materialAO *= (ambientOcclusionBuffer.Sample(defaultSamplerRT, uvScene).r);
#ifdef BNG_ADVANCED_LIGHTING
   outputFinalColor.rgb += (toLinearColor(materialDiffuseColor.rgb) / PI) * calcDiffuseAmbient(getColorSH9(ambientSH9), ambient.rgb, materialNormal, ambient.a).rgb * materialAO;
#else
   outputFinalColor.rgb += toLinearColor(materialDiffuseColor.rgb) * calcDiffuseAmbient(ambient.rgb, materialNormal).rgb * materialAO;
#endif
   ...
   // HDR Output
   outputFinalColor = hdrEncode( float4(outputFinalColor.rgb, outputFinalColor.a) );
   
   // Final color Output
   OUT.target0 = outputFinalColor;
