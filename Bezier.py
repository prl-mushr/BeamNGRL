...
def forward_vector(quat):
	W = np.ones(3)
	W[0] = 1 - 2 * (quat[2]*quat[2] + quat[3]*quat[3])
	W[1] = 2 * (quat[1]*quat[2] + quat[0]*quat[3])
	W[2] = 2 * (quat[1]*quat[3] - quat[0]*quat[2])
	return W

def Bezier_tracker(pos_target, vel_target, speed_target, pos, vel, quat, Tnb, wheelbase=2.6):
	P3 = pos_target - pos
	P0 = np.zeros(3)
	speed = np.linalg.norm(vel)
	Vhat_cur = np.zeros(3)
	if(speed < 0.1):
		Vhat_cur = forward_vector(quat)
	else:
		Vhat_cur = vel/speed
	...
	u = get_T_generic(P0,P1,P2,P3,max(speed,0.1),0.1)
	t = u[0]
	Curvature_t_bf,Direction_t_bf,Normal_t_bf = get_CTN(P0,P1,P2,P3,t)
	C = Curvature_t_bf*Normal_t_bf
	horizontal_curvature = C[1]
	steering = np.arctan(horizontal_curvature * wheelbase)/0.5
	throttle = (speed_target - speed)*0.5 + (speed_target*0.015)
