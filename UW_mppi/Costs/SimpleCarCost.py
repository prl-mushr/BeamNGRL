...
class SimpleCarCost(torch.nn.Module):
    ...
    def __init__(
        ...
        self.NX = max(self.POS.stop, self.YAW.stop, self.VEL.stop, self.STEER.stop, self.SPEED.stop)
    ...
    def forward(self, state):
        ## State is a tensor of shape (bins, samples, horizon, state_dim). If you're not using bins, you can take mean over the first dimension or just use the first bin.
        pos = state[..., self.POS]
        x = pos[..., 0]
        y = pos[..., 1]
        yaw = state[..., self.YAW]
