...
def Plot_metircs(Config):
    ...
    for scenario in Config["scenarios"]:
        ...
        for model in Config["models"]:
            ...
            for trial in range(Config["num_iters"]):
                ...
                cost = (
                    np.clip((1 / ct), 0, 10)
                    + np.clip(np.abs(ay / az_denom), 0, 10)
                    + np.clip(np.abs(az - GRAVITY), 0, 10.0)
                )
    ...
    for model in Config["models"]:
        ...
        axs[0].bar(
            model,
            avg_success_rate_mean,
            yerr=avg_success_rate_std,
            align="center",
            alpha=0.5,
            ecolor="black",
            capsize=10,
        ...
        axs[1].bar(
            model,
            avg_cost_per_unit_time_mean,
            yerr=avg_cost_per_unit_time_std,
            align="center",
            alpha=0.5,
            ecolor="black",
            capsize=10,
    ...
    fig.savefig(
        str(Path(os.getcwd()).parent.absolute())
        + "/Experiments/Results/Control/"
        + "avg_success_rate_cost_per_unit_time.png"
    )
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_name",
        "-c",
        default="Control_Eval_Config.yaml",
        type=str,
