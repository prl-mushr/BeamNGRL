...
def Plot_metircs(Config):
    ...
    plt.subplots_adjust(
        left=0.26, right=0.94, top=1.0, bottom=0.13
    ...
    for model in Config["models"]:
        ...
        plt.bar(
            X + bar_width * (models - model_count - 1),
            time_taken_mean,
            yerr=time_taken_std,
            width=bar_width,
            alpha=1.0,
    ...
    plt.savefig(
        str(Path(os.getcwd()).parent.absolute())
        + "/Experiments/Results/Rollover/In_the_loop.png"
    )
    # plt.show()
    plt.close()
    ...
    plt.savefig(
        str(Path(os.getcwd()).parent.absolute())
        + "/Experiments/Results/Rollover/yaw_alpha.png"
    )
    # plt.show()
    plt.close()
    ...
    plt.subplots_adjust(
        left=0.07, right=1.0, bottom=0.14, top=0.92
    )  # Adjust the values as needed
    for scenario in Config["scenarios"]:
        if scenario == "race-4":
            scn = "Shallow turns"
    ...
    for scenario in Config["scenarios"]:
        ...
        damage_x_OFF = []
        damage_y_OFF = []
        axs[Config["scenarios"].index(scenario)].plot(
            wp_x[::10],
            wp_y[::10],
            linewidth=2,
        ...
        for model in Config["models"]:
            ...
            for trial in range(Config["num_iters"]):
                ...
                x.extend(X[::5].tolist())
                y.extend(Y[::5].tolist())
            axs[Config["scenarios"].index(scenario)].scatter(
                x, y, s=40, label=model_name, alpha=1.0
            )

        # axs[Config["scenarios"].index(scenario)].scatter(start[0], start[1], s=100, label="Start")
        axs[Config["scenarios"].index(scenario)].scatter(
            damage_x_ON, damage_y_ON, s=100, label="Reset for RPS ON", alpha=1.0
        )
        axs[Config["scenarios"].index(scenario)].scatter(
            damage_x_OFF, damage_y_OFF, s=100, label="Reset for RPS OFF", alpha=1.0
        )

        axs[Config["scenarios"].index(scenario)].set_xlabel(
            "X (East) (m)", size="large"
        )
        axs[Config["scenarios"].index(scenario)].set_ylabel(
            "Y (North) (m)", rotation=90, size="large"
        )

        if scenario == "race-4":
            damage_on_shallow = len(damage_x_ON)
            damage_off_shallow = len(damage_x_OFF)
            axs[Config["scenarios"].index(scenario)].legend(
                bbox_to_anchor=(0, 1),
                loc="upper left",
                borderaxespad=0.0,
                fontsize="24",
    ...
    plt.savefig(
        str(Path(os.getcwd()).parent.absolute())
        + "/Experiments/Results/Rollover/trajectories.png"
    )
    # plt.show()
    plt.close(fig)
    ...
    plt.savefig(
        str(Path(os.getcwd()).parent.absolute())
        + "/Experiments/Results/Rollover/rollover_rate_closed.png"
    )
    # plt.show()
    plt.close()
