...
def run_policy(
    ...
    if model == None:
        ...
        action = np.array(
            controller.forward(torch_state).cpu().numpy(), dtype=np.float64
        )[0]
    else:
        action = np.array(
            model.forward(BEV_path, torch_state).cpu().numpy(), dtype=np.float64
        )
    return action
...
def main(config_path=None, hal_config_path=None, args=None):
    ...
    total_experiments = (
        len(Config["policies"]) * len(Config["scenarios"]) * Config["num_iters"]
    )
    experiment_count = 0
            ...
            for scenario in Config["scenarios"]:
                ...
                for trial in range(Config["num_iters"]):
                    ...
                    while ts < time_limit:
                        ...
                        policy_goal = torch.from_numpy(np.copy(goal) - np.copy(pos)).to(
                            device=device, dtype=dtype
                        )
                        action = run_policy(
                            controller,
