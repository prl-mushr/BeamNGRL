...
def create_bng_scenario(
    Map_config, make, model, start_pos, start_quat, args, hal_Config
):
...
def main(config_path=None, hal_config_path=None, args=None):
    ...
    total_experiments = (
        len(Config["vehicle_list"]) * len(Config["scenarios"]) * Config["num_iters"] * 2
    )
    experiment_count = 0
    bng_interface = None
        ...
        for scenario in Config["scenarios"]:
            ...
            for vehicle_name in Config["vehicle_list"]:
                WP_file = (
                    ...
                    + "/Experiments/Waypoints/Rollover-"
                    + scenario
                    + ".npy"
                )
                ...
                bng_interface = create_bng_scenario(
                    Map_config, make, model, start_pos, start_quat, args, hal_Config
                )
                ...
                for attempt in range(speed_iters):
                    for trial in range(num_iters):
                        ...
                        for rollover_prevention in [1]:
                            ...
                            bng_interface.reset(
                                start_pos=trial_pos, start_quat=start_quat
                            )
                            action = np.zeros(2)
                            ...
                            rotate_speed = (
                                1 - f
                            ) * rotate_speed_default + f * rollover_speed_max
                            ...
                            while ts < time_limit:
                                ...
                                action[1] = (
                                    rotate_speed + 2
                                ) / max_speed  ## this is just so that we get to the desired speed quickly
                                if (
                                    state[6] > rotate_speed and not start_turning
                                ):  ## start turning anyway at 5 seconds
                                    start_turning = True
                                    turn_time = ts
                                if start_turning:
                                    action[0] = -1.0 * min(
                                        (ts - turn_time) * 2, 1.0
                                    )  ## swing over 0.5 seconds to full lock
                                else:
                                    action[0] = 0.0
                                ...
                                if (
                                    start_turning
                                ):  ## max value obtained before rollover.
                                    avg_ay.append(
                                        abs(state[10] + track_width * state[12] ** 2)
                                    )
                            ...
                            filename = dir_name + "/{}-{}-{}-{}.npy".format(
                                scenario, vehicle_name, str(trial), str(attempt)
                            )
                            bng_interface.send_ctrl(
                                np.zeros(2),
                                speed_ctrl=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hal_config_name",
        type=str,
        default="combined_rollover.yaml",
        help="name of the config file to use",
    )
    parser.add_argument(
        "--remote",
