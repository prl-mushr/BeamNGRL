...
def Plot_metrics(Config):
    ...
    fig.suptitle(
        "Ratio of Lateral Acceleration to Vertical Acceleration on different surfaces in different vehicles"
    )
    for sc in ["offroad", "flat"]:
        ...
        for vn in ["small car", "big car"]:
            ...
            axs[
                Config["scenarios"].index(scenario),
                Config["vehicle_list"].index(vehicle_name),
            ].set_title("{}-{}".format(sc, vn))
            axs[
                Config["scenarios"].index(scenario),
                Config["vehicle_list"].index(vehicle_name),
            ].set_ylabel("Average Lateral Ratio")
    ...
    plt.savefig(
        ...
        + "/Experiments/Results/Rollover/Results.png"
    )
    plt.show()
    ...
    for vn in ["small car", "big car"]:
        ...
        for scenario in Config["scenarios"]:
            for RP in ["static", "static+dynamic", "none"]:
                ...
                print(
                    "Scenario: {}, Vehicle: {}, Minimum Vertical Acceleration: {}, Rollover Rate: {}".format(
                        scenario, vehicle_name, min_az, rollover_rate
                    )
                )
    axs.legend()
    plt.show()
