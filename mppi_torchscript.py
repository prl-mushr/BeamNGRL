...
class MPPI(torch.nn.Module):
    ...
    def __init__(self, nx, noise_sigma, num_samples=100, horizon=15, device="cuda:0",
        ...
        self.B = torch.tensor(1.53).to(self.d)
        self.C = torch.tensor(1.1).to(self.d)
        self.D = torch.tensor(9.8 * 0.6).to(self.d)
        self.lf = torch.tensor(0.24).to(self.d)
        self.lr = torch.tensor(0.24).to(self.d)
        self.Iz = torch.tensor(0.01).to(self.d)

        self.steering_max = torch.tensor(17.5/57.3).to(self.d)
        self.wheelspeed_max = torch.tensor(17.0).to(self.d)
        self.dt_step = torch.arange(0.02, 0.1, 0.08/self.T).to(self.d)
    ...
    def forward(self, state):
        ...
        action = delta_action*self.dt + state[15:17]
        return action
    ...
    def dynamics(self, state, perturbed_actions, t:int):
        ...
        u0 = torch.clamp(state[:, :, 15] + 10 * perturbed_actions[:,:,0] * self.dt, -1, 1)
        u1 = torch.clamp(state[:, :, 16] + perturbed_actions[:,:,1] * self.dt, -1, 1)
        
        v = torch.sqrt(vx**2 + vy**2)
        Frx = u1*torch.tensor(5.0)
        ...
        vx = vx + ax*self.dt
        vy = vy + ay*self.dt
        gz = gz + self.dt*(Ffy*self.lf*torch.cos(delta) - Fry*self.lr)/self.Iz
        x =  x + (torch.cos(yaw)*vx - torch.sin(yaw)*vy)*self.dt
        y = y + (torch.sin(yaw)*vx + torch.cos(yaw)*vy)*self.dt
        yaw = yaw + self.dt*gz
    ...
    def vectorized_dynamics(self, state, perturbed_actions):
        ...
        curvature = u0 * self.steering_max  # this is just a placeholder for curvature since steering correlates to curvature
    ...
    def running_cost(self, state):
        ...
        state_cost[torch.where(state_cost > 0.99)] = 100
        state_cost = torch.clamp(vx - 10, 0, 10)
