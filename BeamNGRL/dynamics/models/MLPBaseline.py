...
class MLPBaseline(DynamicsBase):
    ...
    def __init__(
        ...
        self.ctrl_dim = 2
        self.context_dim = self.patch_size_px * self.patch_size_px
        self.input_dim = self.context_dim + self.state_dim + self.ctrl_dim
        self.output_dim = 6 ## dvx/dt, dvy/dt, dvz/dt, dwx/dt, dwy/dt, dwz/dt
        self.dt = dt

        patch_fc_layers = [
            nn.Linear(self.patch_size_px*self.patch_size_px, hidden_dim),
            nn.Tanh(),
        ]
        for _ in range(hidden_depth):
            patch_fc_layers += [nn.Linear(hidden_dim, hidden_dim)]
            patch_fc_layers += [nn.Tanh()]
        patch_fc_layers += [nn.Linear(hidden_dim, self.context_dim)]
        patch_fc_layers += [nn.Tanh()]
        self.patch_processor = nn.Sequential(*patch_fc_layers)
    ...
    def CNN(self, x):
        # return self.patch_processor(x.reshape(-1, self.patch_size_px*self.patch_size_px))
        return x.reshape(-1, self.patch_size_px*self.patch_size_px)
