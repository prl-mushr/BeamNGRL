...
class SimpleCarCost(torch.nn.Module):
    ...
    def forward(self, state, controls):
        ...
        roll_cost = torch.clamp((1/ct) - self.critical_SA, 0, 10) + torch.clamp(torch.abs(ay/az) - self.critical_RI, 0, 10) + torch.clamp(torch.abs(az - self.GRAVITY) - 3.0, 0, 10.0) + torch.clamp(torch.abs(vz) - 0.15, 0, 10.0)
